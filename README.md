...
| **gene_expression_nodes**      | ***list, default = []*** <br> A list of predictors (e.g. TFs) to use that typically is found in the training gene expression data $X_{train}$. <br> This is *optional* but may boost the speed of training and fitting NetREm models (by adjusting the network prior). If not specified, then NetREm will automatically determine gene_expression_nodes when fitting the model with $X_{train}$ data (but will need time to recalibrate the network prior). |
